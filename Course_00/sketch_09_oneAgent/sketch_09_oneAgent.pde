...
void setup()
{
  ...
  //Remplir le tableau de nouvelle balles à des positions aléatoires//Soit maxDistToDraw la distance minimal pour commencer à dessiner une ligne
  maxDistToDraw = random(10, 100);
  nombreMaximumDeBalle = round(random(50, 100));
  for (int i = 0; i<nombreMaximumDeBalle; i++)
  {
   listBall.add(new Ball(random(width/2-100, width/2+100), random(height/2-100, height/2+100))); //nom du tableau . ajoute (nouvelle balle(à positionx, à position y));
  }
...
void draw()
{
  ...
  for (int i=0; i<listBall.size (); i++)
  {
    ...
    for (int j=i+1; j<listBall.size (); j++)
    {
      ...
      //Soit c la couleur proportionnelle à la distance entre les balles (de 0 pixel à maxDistToDraw pixels des distance)
      float c = map(distance, 0, maxDistToDraw, 50, 0);
  ...
  if (mousePressed)
  {
    //Alors on vide le tableau
    listBall.clear();
    //On dessine un background pour effacer le dessin precedent
    background(0, 0, 200);
    //On change le nombre max de balles pouvant être présente dans le tableau
    nombreMaximumDeBalle = round(random(50, 100));
    //on change la distance minimale pour dessiner des lignes
    maxDistToDraw = random(10, 100);
    //On re remplit le tableau de balles à des positions au hasard
    for (int i = 0; i<nombreMaximumDeBalle; i++)
    {
      listBall.add(new Ball(random(width/2-100, width/2+100), random(height/2-100, height/2+100))); //nom du tableau . ajoute (nouvelle balle(à positionx, à position y));
    }
